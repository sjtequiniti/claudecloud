...
These commands govern every session. Execute them before any other work.

- **Session start**: Read CLAUDE.md, README.md, and plans/TODO.md to determine current status. Say "Ready to resume" and propose the next action. Do this BEFORE responding to the user's first message.
...
tmp/                   <-- Temporary artefacts, not under version control
```

## Development commands

### Testing
- Test files should be named `test_*.*`
- Unit tests go in: `tests/`
- For ad-hoc testing, write scripts to `tmp/` in the project directory (never committed)
- Don't use `/tmp` or other locations outside the repository

### Code formatting

Use linter on all scripts.

## Implementation guidelines

### Project rules
1. **No workarounds** - Fix root causes, not symptoms
2. **CRITICAL: All imports, inclusions, etc. at file top** - NO conditional imports anywhere, including tests. Import everything needed upfront, fail fast on missing dependencies
...
2. Always include unit tests alongside code changes
...
Important: You MUST stop for reviews before ANY implementation.
